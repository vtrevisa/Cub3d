...
int	parse_config_file(t_data *data)
{
	...
	if (data->col_ok != 2)
		return (col_error());
	data->line_below = malloc(data->map_size[0] + 2);
	ft_bzero(data->line_below, data->map_size[0] + 2);
	line_guide = malloc(data->map_size[0] + 2);
	ft_bzero(line_guide, data->map_size[0] + 2);
